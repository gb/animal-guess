...
class SwingAdapter extends Object with UIAdapter {
  	...
  	def makeQuestion(questionable: Questionable) : Boolean = 
  		return JOptionPane.showConfirmDialog(null, questionable.question(), GAME_NAME, JOptionPane.YES_NO_OPTION) == 0

	def makeTextQuestion(question: String) = JOptionPane.showInputDialog(question)
